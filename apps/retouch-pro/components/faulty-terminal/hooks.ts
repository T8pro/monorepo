...
export const useFaultyTerminal = (props: Partial<FaultyTerminalProps>) => {
  const {
    scale = 1,
    gridMul = [2, 1],
    digitSize = 1.5,
  ...
  useEffect(() => {
    if (programRef.current) {
      programRef.current.uniforms.uTint.value = new Color(
        tintVec[0],
        tintVec[1],
        tintVec[2],
      );
      programRef.current.uniforms.uBackground.value = new Color(
        backgroundVec[0],
        backgroundVec[1],
        backgroundVec[2],
      );
